...
<script>
	...
	$(function(){
		$("#selectAll").click(function() {
			//这里也可以直接用this,原生js语句 if (this.checked)
			//或$(this).attr('checked')
			if ($(this)[0].checked) {
		...
		$("#delselectusers").click(function() {
			...
			//提交表单
			$("#myform").action = "optUsersAction_delUsersBySelect";//更改提交路径
			$("#myform").submit();
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
         ...
         <!-- ================================注意表单================================= -->
     	 <form action="selectUsersAction_selectusers" method="post">
            
            <tr>
              <td colspan="4" background="/mywebtraining/images/ht-body02.jpg" class="body03">查询条件</td>
            </tr>
            <tr> 
              <td width="25%" class="body06">学员编号：</td>  <!-- 编号是唯一的，回显没有意义，算了还是加上,注意users.u_id不要加引号'users.u_id':'' -->
              <td width="27%" class="body04"><input name="users.u_id" id="uid" value="${users.u_id!=0? users.u_id:'' }" type="text" class="bodytext" /></td>
     ...
     <c:if test="${not empty userslist }">
      ...
      <form action="optUsersAction_delUsersBySelect?&users.u_id=${users.u_id }&users.dep_id=${users.dep_id }&users.edu_id=${users.edu_id }&users.major_id=${users.major_id }&users.u_name=${users.u_name }&users.u_sex=${users.u_sex }&u_birth1=${u_birth1 }&u_birth2=${u_birth2 }&active_time1=${active_time1 }&active_time2=${active_time2 }&page=${page }" method="post">
     	...
     	<c:forEach items="${userslist }" var="myusers" varStatus="idx" >
			...
			<!-- 注意此处是type="image"会影响"选择删除"表单的提交，要加上return false -->
			<input type="image" src="/mywebtraining/images/ht-but-del.jpg" 
				onclick="window.location='optUsersAction_delOneusers?operusers.u_id=${myusers.u_id }&users.u_id=${users.u_id }&users.dep_id=${users.dep_id }&users.edu_id=${users.edu_id }&users.major_id=${users.major_id }&users.u_name=${users.u_name }&users.u_sex=${users.u_sex }&u_birth1=${u_birth1 }&u_birth2=${u_birth2 }&active_time1=${active_time1 }&active_time2=${active_time2 }&page=${page }';return false;"/>
			
			
			<input type="image" src="/mywebtraining/images/ht-but-adit.jpg" 
				onclick="window.location='optUsersAction_selectOneUsers?operusers.u_id=${myusers.u_id }&u_id=${users.u_id }&dep_id=${users.dep_id }&edu_id=${users.edu_id }&major_id=${users.major_id }&u_name=${users.u_name }&u_sex=${users.u_sex }&u_birth1=${u_birth1 }&u_birth2=${u_birth2 }&active_time1=${active_time1 }&active_time2=${active_time2 }&page=${page }';return false;"/>
